    ...
    if (filters.citySearchLocation) {
      result = result.map(museum => ({
        ...
        distance: calculateDistance(
          filters.citySearchLocation!.latitude,
          filters.citySearchLocation!.longitude,
          museum.latitude,
          museum.longitude
        )
      }));
      ...
      // Filter by radius
      result = result.filter(museum =>
        museum.distance !== undefined && museum.distance <= filters.citySearchRadius
      );
    ...
    <div className="space-y-6">
      ...
      <div>
          ...
          <div>
            {filters.citySearchLocation ? (
              <p className="text-sm text-muted-foreground">
                Showing {filteredAndSortedMuseums.length} museums within {filters.citySearchRadius} miles of {filters.citySearchLocation.city}
              </p>
            ) : (
              <p className="text-sm text-muted-foreground">
        ...
        {filteredAndSortedMuseums.length === 0 ? (
          ...
          <MapView
            museums={filteredAndSortedMuseums}
            showDistance={true}
          />
        ) : (
